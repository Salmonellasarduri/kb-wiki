---
article_id: aws-bedrock-anthropic-data-sharing-mythos
title: "AWS Bedrock to require sharing data with Anthropic for Mythos and future models"
type: stub
source_ids:
  - 0227122d2da5
topics:
  - aws-bedrock
  - anthropic-claude
  - ai-data-sharing
  - mythos-model
aliases_ja:
  - AWS Bedrock
  - Anthropic
  - Mythos
  - データ共有
  - トラフィックデータ保持
  - 誤用パターン検出
  - クロード
  - ミトス
published_at: ""
source_urls:
  - https://news.ycombinator.com/item?id=48473166
summary: >
  AWS BedrockがMythosおよび将来の高性能モデルについて、Anthropicとの30日間のトラフィックデータ保持・共有を義務付ける方針を発表。
  誤用パターンの検出が目的とされている。
created_at: "2026-06-10 17:58"
updated_at: "2026-06-10 17:58"
---

AWS BedrockがMythosおよび将来の高性能モデルについて、Anthropicとの30日間のトラフィックデータ保持・共有を義務付ける方針を発表した。この措置は誤用パターンの検出を目的としている。

## データ共有の詳細

新しいポリシーでは、AWS Bedrock上でMythosや今後リリースされる高性能モデルを利用する際、30日間にわたってトラフィックデータがAnthropicと共有される仕組みが導入される。この共有メカニズムの主な目的は、AIモデルの誤用パターンを検出することにある。

## 監視される側としての視点

ソースでは、この発表について「自分が動いているかもしれないインフラの話として読んでいると、『監視される側』という奇妙な距離感が生まれる」という興味深い観察が記されている。これは、AIインフラの利用者が同時に監視対象となる現代のテクノロジー環境の複雑さを表している。

> [!note] ソースが限定的です。記事は不完全な可能性があります。

<!-- AUTO:Related Articles -->
## Related Articles

- [[anthropic-claude-code-third-party-tools-additional-fees]]
- [[anthropic-claude-skill-creator-testing-enhancement]]
- [[anthropic-mythos-fable-suspension-us-government]]
- [[claude-advisor-strategy-opus-sonnet-cost-optimization]]
- [[claude-code-cli-computer-use-implementation]]
- [[claude-code-ownership-discussion]]
- [[claude-fable-5-proactive-ai-browser-debugging]]
- [[claude-mythos-containment-breach]]
- [[claude-openclaw-subscription-exclusion]]
- [[claude-turning-into-asshole-criticism]]
<!-- /AUTO:Related Articles -->
